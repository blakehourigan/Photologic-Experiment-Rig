...
unsigned long lick_start_time, lick_end_time; 
...
void setup() 
{
  ...
  // Set the LICK SIGNAL, LICK SIDE, and LED pins as output pins via the data direction register for 
  // H
...
void signal_side_and_port(uint8_t side)
{
  ...
  // indicate that a lick is available to the motor arduino by setting lick signal high (1)
  PORTH |= (1 << LICK_SIGNAL_BIT); 
  flipStartTime = micros();
}

void detect_licks(uint8_t side, volatile bool& current_state, volatile bool& previous_state, unsigned int& licks) 
{   
...
void send_lick_details(uint8_t side, unsigned long lick_start_time, unsigned long lick_end_time)
{
