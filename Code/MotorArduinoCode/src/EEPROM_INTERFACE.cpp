...
void EEPROM_INTERFACE::write_values_to_EEPROM(unsigned long int values[], int start_address, int num_values) {
    for (int i = 0; i < num_values; i++) {
        EEPROM.put(start_address + i * sizeof(long int), values[i]);
    }
}

void EEPROM_INTERFACE::read_values_from_EEPROM(unsigned long int values[], int start_address, int num_values, bool print) {
    for (int i = 0; i < num_values; i++) {
...
void EEPROM_INTERFACE::read_single_value_from_EEPROM(unsigned long int &value, int start_address, int index, bool print) {
