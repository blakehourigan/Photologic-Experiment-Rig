...
class valveTestLogic:
    ...
    def start_valve_test_sequence(self, num_valves):
        ...
        execute = self.check_container_measurements(self.side_one_container_measurement, self.side_two_container_measurement)
        logging.debug(f"Container measurements obtained: Side 1: {self.side_one_container_measurement}, Side 2: {self.side_two_container_measurement}, Execute: {execute}")
        ...
        if execute:
            ...
            logging.warning("Container measurements invalid, aborting valve test sequence.")
    ...
    def append_to_volumes(self) -> None:
        ...
        logging.debug(f"Appended volumes: Side 1: {self.side_one_volumes[-1]}, Side 2: {self.side_two_volumes[-1]}")

        execute = self.check_container_measurements(self.side_one_container_measurement, self.side_two_container_measurement)
        logging.debug(f"Container measurements check after appending volumes: Execute: {execute}")
    ...
    def update_opening_times(self, num_valves, durations, volumes) -> Tuple[List[int], List[float]]:
        ...
        logging.debug(f"Updated opening times: {opening_times}, Ul per lick: {ul_dispensed}")
        return opening_times, ul_dispensed

    def check_container_measurements(self, side_one, side_two) -> bool:
        logging.debug(f"Checking container measurements: Side 1: {side_one}, Side 2: {side_two}")
        execute = True
